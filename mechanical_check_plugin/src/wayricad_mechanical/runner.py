...
def run(board_path, config, progress=lambda value, message: None, cancel=None, project_dir=None):
    ...
    runtime = discover()
    for key, description in [('kicad_python', 'KiCad Python'), ('kicad_cli', 'KiCad CLI'), ('freecad_python', 'FreeCAD Python')]:
        if not runtime[key]:
    ...
    with tempfile.TemporaryDirectory(prefix='wayricad-mechanical-') as temp:
        ...
        result['export_log'] = (work / 'export.log').read_text(encoding='utf-8', errors='replace')
