...
def screen(path, *, rise_ns=1., frequency_mhz=100., source_ohm=20., load_ohm=None,
           z0_ohm=None, epsilon_eff=None, terminal_count=2):
    ...
    return {'schema':'wayricad.quick-si/v1','status':'UNRESOLVED' if not resolved else 'SCREENED' if delay is not None and z0 is not None else 'INCOMPLETE',
...
def html_report(report):
    ...
    rows=''.join('<tr><th>'+escape(key.replace('_',' '))+'</th><td>'+escape('Unknown' if value is None else value)+'</td></tr>'
                 for key,value in report.items() if key not in ('path','inputs','notes','schema'))
    points=[]
    ...
    return '<!doctype html><meta charset="utf-8"><title>WayriCAD Quick SI</title><style>body{font:15px system-ui;max-width:950px;margin:30px auto;color:#20303c}td,th{padding:7px;text-align:left;border-bottom:1px solid #ddd}pre{white-space:pre-wrap}</style><h1>WayriCAD Quick SI</h1><p>Read-only first-order screening, not protocol signoff or an eye simulation.</p>'+svg+'<table>'+rows+'</table><h2>Assumptions and limitations</h2><ul>'+''.join('<li>'+escape(n)+'</li>' for n in report['notes'])+'</ul><h2>Inputs and path evidence</h2><pre>'+escape(json.dumps({'inputs':report['inputs'],'path':report['path']},indent=2,allow_nan=False))+'</pre>'
