"""First-order transmission-line approximations for routed copper geometry.
...
def solve(
    ...
    """Return a labelled first-order model bundle for one uniform section."""
...
def z0_width_sweep(
    ...
    for w in widths:
        try:
            points.append((w, solver(w, height_mm, copper_mm, relative_permittivity)))
        except ValueError:
            continue  # Exclude invalid geometries rather than plotting a false zero.
    return points
