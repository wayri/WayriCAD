...
class TestPointFrame(wx.Panel):
    def __init__(self, parent: Any, board: Any) -> None:
        ...
        for label, handler in (("Refresh Preview", self.extract), ("Export…", self.export_menu)):
            ...
            if label == "Export…":self.export_button=button
        ...
        panel.SetSizer(root)
        host=wx.BoxSizer(wx.VERTICAL);host.Add(panel,1,wx.EXPAND);self.SetSizer(host)
        self.extract(None)
        for control in (self.field,self.boards):control.Bind(wx.EVT_TEXT,self.invalidate)
        self.consolidate.Bind(wx.EVT_CHECKBOX,self.invalidate)

    def invalidate(self,event=None):
        self.rows=[];self.list.DeleteAllItems();self.export_button.Disable();self.status.SetLabel('Settings changed. Refresh preview before exporting.')
        if event:event.Skip()
    ...
    def _refresh_list(self) -> None:
        ...
        self.export_button.Enable(bool(self.rows))
    ...
    def generate_fixture_pcb(self, _event: Any) -> None:
        ...
        if wx.MessageBox(message, "Generate reviewed bed-of-nails fixture", wx.YES_NO | wx.NO_DEFAULT | wx.ICON_WARNING, self) != wx.YES:
            return
        with wx.FileDialog(self, "Write bed-of-nails fixture PCB", defaultDir=str(Path(self.board.GetFileName()).parent), defaultFile="test-fixture.kicad_pcb", wildcard="KiCad PCB (*.kicad_pcb)|*.kicad_pcb", style=wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT) as dialog:
            if dialog.ShowModal() != wx.ID_OK:
                return
            if Path(dialog.GetPath()).resolve()==Path(self.board.GetFileName()).resolve():
                wx.MessageBox('Choose a separate fixture board; the source PCB must remain unchanged.','Fixture export',wx.OK|wx.ICON_ERROR,self);return
    ...
    def select_test_point(self, event: Any) -> None:
        ...
        try:
            ...
            targets=[fp.m_Uuid.AsString() for fp in self.board.GetFootprints() if fp.GetReference() in references]
            count=select_origin(self.board,targets)
            self.status.SetLabel(f'Selected {count} footprint(s) in the originating PCB.')
        except Exception as exc:
            from wayricad_runtime.context import redact_error
            self.status.SetLabel(redact_error(exc))
        self.workflow.set_step(2, "Continue reviewing records or export the approved document.")

    def _save_path(self, wildcard: str) -> str:
        with wx.FileDialog(self, "Export test-point documentation", defaultDir=str(Path(self.board.GetFileName()).parent), wildcard=wildcard, style=wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT) as dialog:
            if dialog.ShowModal() != wx.ID_OK:
                return ""
            destination=Path(dialog.GetPath()).resolve()
            if destination.suffix.lower().startswith('.kicad_'):
                wx.MessageBox('Choose a report filename, not a KiCad design file.','Export',wx.OK|wx.ICON_ERROR,self);return ''
