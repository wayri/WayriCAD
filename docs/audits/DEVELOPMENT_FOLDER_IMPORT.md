...
Imported three distinct applications from the user-provided `D:/PROJECTS-DEV/Kicad-plugins-dev` directory, bringing the WayriCAD 3.0.0 testing suite to 22 independent PCM packages. Source directories were read only. Git internals, caches, large third-party examples, generated reports and old build products were excluded. Each imported directory records source provenance and retains available license notices.
