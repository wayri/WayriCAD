...
def main():
 ...
 try:
  with sync_playwright() as p:
   ...
   page.locator('[data-view=assemblers]').click();page.wait_for_function('ASM.defs && Object.keys(ASM.defs).length===8');check(page.locator('[data-asm-profile]').count()==8,'All eight assembler cards included')
   ...
   d=info.value;dest=out/d.suggested_filename;d.save_as(dest);z=zipfile.ZipFile(dest);check(len([n for n in z.namelist() if n.startswith('uploads/')])==16,'ZIP has all eight CSV/Excel alternatives');check('reports/EXCLUDED.csv' in z.namelist(),'DNP/excluded audit accompanies ZIP');check(b'PER BOARD' in z.read('README_UPLOAD.txt'),'Upload instructions protect quantity interpretation')
   page.locator('[data-view=bom]').click();page.locator('[data-view=assemblers]').click();check(page.evaluate('ASM.config.profiles.length')==8,'Unsaved selections survive navigation');page.locator('#saveButton').click();page.wait_for_function('!ASM.draft && !S.data.dirty');check(ws.state['assembler_export_settings']['profiles']==list(__import__('bomstudio.assembler_export',fromlist=['PROFILES']).PROFILES),'Top Save persists assembly profile selections');check(page.evaluate('followingNative()'),'Assembly settings do not switch native BOM authority');check(ws.state['vendor_export_settings']==vendors_before,'Distributor settings remain unchanged')
