...
class NativeFirstInterfaces(Fixture):
 ...
 def test_http_native_mode_by_default(self):
  code,raw=self.request('state');d=json.loads(raw);self.assertEqual(code,200);self.assertEqual(d['bom_authority']['preferences']['mode'],'native');self.assertEqual(d['runtime']['version'],json.loads((Path(__file__).resolve().parents[1]/'metadata.json').read_text(encoding='utf-8'))['versions'][0]['version'])
