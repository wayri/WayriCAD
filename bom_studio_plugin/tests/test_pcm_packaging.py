...
class PCMPackageTests(unittest.TestCase):
 ...
 def test_valid_flat_package(self):
  r=v.validate(self.archive());self.assertEqual(r['version'],'3.0.0');self.assertFalse(r['host_installation_tested']);self.assertEqual(r['installed_plugin_relative_path'],'plugins/com_github_wayri_wayricad_bom-studio/plugin.json')
